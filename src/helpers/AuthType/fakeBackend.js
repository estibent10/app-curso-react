...
const fakeBackend = () => {
  ...
  mock.onGet(url.GET_RECENT_USERS).reply(() => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (recentUsers) {
          // Passing fake JSON data as response
          resolve([200, recentUsers]);
        } else {
          reject([400, "Cannot get recentUsers"]);
        }
      });
    });
  });

  mock.onGet(url.GET_LATEST_TRANSACTION).reply(() => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (latestTransaction) {
          ...
          reject([400, "Cannot get Latest Transaction"]);
        }
      });
    });
  });

  mock.onGet(url.GET_LATEST_ORDERS).reply(() => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (latestOrders) {
          // Passing fake JSON data as response
          resolve([200, latestOrders]);
        } else {
          reject([400, "Cannot get Latest Orders"]);
        }
      });
    });
  });
